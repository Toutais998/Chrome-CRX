...
(function () {
    ...
    const styles = `
        ...
        .if-badge, .jcr-badge, .rank-badge {
            ...
            background: linear-gradient(135deg, #95a5a6, #7f8c8d); /* Default gray gradient */
        }
    ...
    async function getEnhancedJournalInfo(journalName, journalData) {
        ...
        for (const publisher of PUBLISHER_PATTERNS) {
            if (publisher.journals && publisher.journals.some(j => j.toLowerCase() === normalizedName)) {
                // 如果在已知的高IF期刊列表中找到
                return {
                    name: journalName,
                    if: "预设高IF期刊",
    ...
    function renderPanelContent(article, journalInfo) {
        ...
        if (journalInfo) {
            ...
            if (journalInfo.if5y) { impactHTML += ` <span style="margin: 0 4px;">5Y IF:</span><span class="if-badge">${journalInfo.if5y}</span>`; }
    ...
    async function displayAcademicInfo(journalData, sciHubDomainPromise) {
        // 增加一个初步的学术页面检测，如果连DOI都提取不到，则大概率不是学术页面
        const doi = extractDOI();
        if (!doi) {
            console.log("Academic Info Displayer: No DOI found on this page. Aborting.");
            return;
        }

        const getMetaContent = (name) => document.querySelector(`meta[name='${name}']`)?.content || '';
        let article = { doi: doi, title: '', journal: '', year: '', authors: [] };
        let apiSucceeded = false;

        // 使用增强的DOI提取方法
        // article.doi = extractDOI(); // DOI is already extracted
